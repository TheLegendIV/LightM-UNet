...
static int TtcPs_Init_1ms_any(void)
{
    ...
    /* Device 0 is already claimed by platform_zynqmp.c's GIC-interrupt-driven
     * timer (init_platform() -> platform_setup_timer()/platform_setup_interrupts(),
     * TIMER_DEVICE_ID = XPAR_XTTCPS_0_DEVICE_ID). Reusing it here would let that
     * ISR clear the interval-interrupt status bit before this polling code ever
     * observes it, so ms_ticks (and sys_now()) would never advance -- start the
     * search at index 1 to grab a different, unclaimed TTC counter. */
    for (int i = 1; i < 12; ++i) {
