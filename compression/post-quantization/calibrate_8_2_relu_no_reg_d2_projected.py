"""Builds nnUNetTrainerENet_8_2_relu_no_reg_d2_projected's own real HAWQ
...
saving -- confirmed load-bearing by this session's own local proxy test
(training loss was noisy/non-monotonic without calibration, clean and
monotonically decreasing with it; see compression/qat_smoke_8_2_relu_no_reg_
d2_projected.py). Calibration (Brevitas's calibration_mode(), forward-only,
no gradients) sets each quantizer's scale from real observed activation
ranges instead of Brevitas's arbitrary PARAMETER_FROM_STATS default --
conv/BN weights transfer unchanged (real name+shape match via
CombinedQuantENet.from_pretrained, same generic mechanism every per-block/
per-stage quant checkpoint in this repo already uses).

Output checkpoint is meant as the WARM START for
nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_d2_projected's own real QAT
fine-tuning (ENET_PRETRAINED_CHECKPOINT), not a final deployable artifact on
its own -- see compression/slurm/qat_8_2_relu_no_reg_d2_projected_10ep.job.

Usage:
    python compression/post-quantization/calibrate_8_2_relu_no_reg_d2_projected.py \\
        --stage-bits-file compression/hawq/artifacts/stage_bits_8_2_relu_no_reg_d2_projected_minres.json \\
        --out-net-name nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_d2_projected_calibrated
"""
...
def main() -> None:
    ...
    parser.add_argument("--source-net-name", default="nnUNetTrainerENet_8_2_relu_no_reg_d2_projected")
    ...
    new_checkpoint["trainer_name"] = "nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_d2_projected"
