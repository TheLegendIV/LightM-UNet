"""Builds nnUNetTrainerENet_8_2_relu_w16_no_reg_context_only's own real
per-BLOCK HAWQ-quantized CombinedQuantENet from its trained FP32
checkpoint, then CALIBRATES the newly-introduced Brevitas quantizers on
real data before saving -- same rationale/mechanism as
calibrate_8_2_relu_no_reg_w16_perblock.py (see that file's own module
docstring for the full calibration-load-bearing rationale -- not repeated
here).

Differs from that UNSCOPED sibling in exactly one architecture flag:
dsc_no_projection_context_only=True -- only stage2/stage3's dilated slots
become DSCNoProjectionBottleneck; regular1/regular4/regular5 stay REAL
projected RegularBottleneck blocks, matching the FP32 source checkpoint's
own architecture. Same channels=(4,8,16,8,4) ("w16" width) otherwise.

block_names_for(BOTTLENECKS_PER_STAGE)'s own naming convention already
matches block_utils.enumerate_blocks's naming exactly -- so no
expand_stage_bits() broadcast step is needed: any block_bits_*.json's own
"stage_weight_bits"/"stage_act_bits" dicts are ALREADY keyed by block name
and can be passed straight through, same as every other w16/w20 per-block
calibration script this session.

Output checkpoint is meant as the WARM START for
nnUNetTrainerCombinedQuantENet_8_2_relu_w16_no_reg_context_only_perblock's
...
def main() -> None:
    ...
    parser.add_argument("--source-net-name", default="nnUNetTrainerENet_8_2_relu_w16_no_reg_context_only")
    ...
    new_checkpoint["trainer_name"] = "nnUNetTrainerCombinedQuantENet_8_2_relu_w16_no_reg_context_only_perblock"
