"""Builds nnUNetTrainerENet_12_separable_dense_relu_warmstart150ep's own real
...
saving -- byte-for-byte the same mechanism as
calibrate_12_dense_relu_warmstart150ep_perlayer.py, just for the WARM-STARTED
12_separable_dense_relu architecture (separable_dilated=True -- dense conv
factored into a (k,1)+(1,k) pair, plain "dense_dilation" context pattern, no
reg-trailing block -- see compression/hawq/config_12_separable_dense_relu_warmstart150ep.py).

Output checkpoint is meant as the WARM START for
nnUNetTrainerLayerQuantENet_12_separable_dense_relu_warmstart150ep_perlayer's
own real QAT fine-tuning (ENET_PRETRAINED_CHECKPOINT), not a final deployable
artifact on its own.

Usage:
    python compression/post-quantization/calibrate_12_separable_dense_relu_warmstart150ep_perlayer.py \\
        --layer-bits-file compression/hawq/artifacts/12_separable_dense_relu_warmstart150ep_ILP_outputs_perlayer_forcedsp_lut70/layer_bits_SITES_12_separable_dense_relu_warmstart150ep_joint_alpha0.5_candidatebits468_forcedsp_lut70.json \\
        --out-net-name nnUNetTrainerLayerQuantENet_12_separable_dense_relu_warmstart150ep_joint_alpha0.5_candidatebits468_calibrated
"""
...
def main() -> None:
    ...
    parser.add_argument("--source-net-name", default="nnUNetTrainerENet_12_separable_dense_relu_warmstart150ep")
    ...
    new_checkpoint["trainer_name"] = "nnUNetTrainerLayerQuantENet_12_separable_dense_relu_warmstart150ep_perlayer"
