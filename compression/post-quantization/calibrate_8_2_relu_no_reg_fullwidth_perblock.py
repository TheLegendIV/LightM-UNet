"""Builds nnUNetTrainerENet_8_2_relu_no_reg_fullwidth's own real per-BLOCK
...
saving -- same rationale/mechanism as calibrate_8_2_relu_no_reg_fullwidth.py
(confirmed load-bearing there: loss noisy/non-monotonic without
calibration, clean and monotonically decreasing with it), just consuming a
per-block bit assignment (compression/hawq/block_bits_8_2_relu_no_reg_
fullwidth_acc1x_joint.json -- one entry PER INDIVIDUAL bottleneck block,
e.g. "stage2.3", not one shared pair per 5-way stage group) instead of
calibrate_8_2_relu_no_reg_fullwidth.py's own per-STAGE bits.

block_names_for(BOTTLENECKS_PER_STAGE)'s own naming convention (confirmed
this session) already matches block_utils.enumerate_blocks's naming
exactly (the same function the real HAWQ per-block search used to produce
block_bits_8_2_relu_no_reg_fullwidth_acc1x_joint.json in the first place)
-- so unlike the per-stage sibling, no expand_stage_bits() broadcast step
is needed here at all: the JSON's own "stage_weight_bits"/"stage_act_bits"
dicts (named that way only because ilp_search.py's output schema is
granularity-agnostic -- see that file's own module docstring) are ALREADY
keyed by block name and can be passed straight through.

Output checkpoint is meant as the WARM START for
nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_fullwidth_perblock's own
real QAT fine-tuning (ENET_PRETRAINED_CHECKPOINT), not a final deployable
artifact on its own -- see compression/slurm/qat_8_2_relu_no_reg_fullwidth_
perblock_acc1x_15ep.job.
...
def main() -> None:
    ...
    new_checkpoint["trainer_name"] = "nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_fullwidth_perblock"
