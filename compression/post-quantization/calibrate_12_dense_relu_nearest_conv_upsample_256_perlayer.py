"""Builds nnUNetTrainerENet_12_dense_relu_nearest_conv_upsample_256's own real
...
saving -- byte-for-byte calibrate_12_dense_relu_nearest_conv_upsample_
perlayer.py's own mechanism, for the 256x256 (Dataset510_ARCADE_256_4c,
CONTEXT_PATTERN="dense_dilation_half") counterpart architecture -- see
nnUNetTrainerLayerQuantENet_12_dense_relu_nearest_conv_upsample_256_perlayer.py's
own module docstring for why only CONTEXT_PATTERN differs from the 512x512
family (dilation never changes a conv's weight tensor shape, so quantizer
site names/shapes are identical either way).

Output checkpoint is meant as the WARM START for
nnUNetTrainerLayerQuantENet_12_dense_relu_nearest_conv_upsample_256_perlayer's
own real QAT fine-tuning (ENET_PRETRAINED_CHECKPOINT), not a final
deployable artifact on its own.

Usage:
    python compression/post-quantization/calibrate_12_dense_relu_nearest_conv_upsample_256_perlayer.py \\
        --layer-bits-file MILP/artifacts/S12_dense_nn_upsample_256_v1/layer_bits_SITES_12_dense_relu_nearest_conv_upsample_256_joint_alpha1.0_candidatebits468_forcedsp_lut50_bram50_dsp90_fps250.json \\
        --out-net-name nnUNetTrainerLayerQuantENet_12_dense_relu_nearest_conv_upsample_256_joint_alpha1.0_candidatebits468_lut50_bram50_dsp90_fps250_calibrated
"""
...
def main() -> None:
    ...
    parser.add_argument("--source-net-name", default="nnUNetTrainerENet_12_dense_relu_nearest_conv_upsample_256")
