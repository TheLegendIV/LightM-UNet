"""Builds nnUNetTrainerENet_27_2_reg_trailing's own real per-BLOCK
...
saving -- same rationale/mechanism as calibrate_12_separable_dense_relu_
perblock.py (confirmed load-bearing there: loss noisy/non-monotonic
without calibration, clean and monotonically decreasing with it), for
S27.2's own architecture: S12's separable_dilated=True/use_dsc=False/
dsc_no_projection=False recipe PLUS a real projected RegularBottleneck
("trailing consolidation" block) inserted after each (2,4,8,16) dilation
cycle -- context_pattern="dense_dilation_reg_trailing" (ENet.py's own
DENSE_DILATION_REG_TRAILING_PATTERN), stage2/3 bottleneck depth 10 (not
S12's native 8). Required CombinedQuantENet.py to learn this context
pattern for the first time this session (it previously only supported the
subset QuantENet._make_context_stage already had) -- see that file's own
_make_block_context_stage docstring/comments for the reg-bookend handling
...
the real HAWQ per-block search used to produce block_bits_27_2_reg_
trailing_min4.json in the first place) -- so no expand_stage_bits()
...
nnUNetTrainerCombinedQuantENet_27_2_reg_trailing_perblock's own real QAT
fine-tuning (ENET_PRETRAINED_CHECKPOINT), not a final deployable artifact
on its own -- see compression/slurm/qat_27_2_reg_trailing_min4_ft15ep.job.
...
def main() -> None:
    ...
    new_checkpoint["trainer_name"] = "nnUNetTrainerCombinedQuantENet_27_2_reg_trailing_perblock"
