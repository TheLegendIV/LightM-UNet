"""Builds nnUNetTrainerENet_8_2_relu_no_reg_w20's own real per-BLOCK HAWQ-
quantized CombinedQuantENet from its trained FP32 checkpoint, then
...
perblock.py (see that file's own module docstring for the full
calibration-load-bearing rationale -- not repeated here), just at this
net's own channels=(4,10,20,10,4) width and consuming compression/hawq/
block_bits_8_2_relu_no_reg_w20_acc1x_joint.json (real per-block sensitivity,
found Optimal: all 16 context blocks at uniform 2-bit weights, mixed
2/4-bit activations, 100.0% LUT / 25.8% BRAM / ~23.0ms hard-capped folding
-- the fastest deployable result of any S8.2 variant checked this session).

block_names_for(BOTTLENECKS_PER_STAGE)'s own naming convention already
matches block_utils.enumerate_blocks's naming exactly (the function the
real HAWQ per-block search used to produce that JSON in the first place) --
so, same as the fullwidth sibling, no expand_stage_bits() broadcast step is
needed: the JSON's own "stage_weight_bits"/"stage_act_bits" dicts are
ALREADY keyed by block name.

Output checkpoint is meant as the WARM START for
nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_w20_perblock's own real QAT
fine-tuning (ENET_PRETRAINED_CHECKPOINT), not a final deployable artifact
on its own -- see compression/slurm/qat_8_2_relu_no_reg_w20_perblock_acc1x_15ep.job.
...
def main() -> None:
    ...
    parser.add_argument("--source-net-name", default="nnUNetTrainerENet_8_2_relu_no_reg_w20")
    ...
    new_checkpoint["trainer_name"] = "nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_w20_perblock"
