...
stage_18_reginterleaved_separable_nonneg_block,
stage_19_reginterleaved_separable_nonneg_block_double_mid, and
stage_26_s5_6_probe_family config -- i.e. everything actually built off that
one reference point, with the rest of the sweep (Base/U2/U3/U6/U8/U16/E1,
stage_2_special_ops, stage_3_transfer_original) filtered out as noise for
this comparison. stage_26 is two hops removed (U4 -> S5.6 -> the 8 stage_26
variants), not a direct U4 probe, but it's still the same lineage -- see
compression/finn_cost_s5_6_variants.py/stage_26_s5_6_probe_family_array.job
for how those 8 configs were derived from S5.6. A horizontal dashed line at
U4's own dice marks the bar every probe is trying to clear.
...
def main() -> int:
    ...
    ax.set_title("U4 reference + all U4-lineage architecture probes (stages 4-20, 26): Dice vs. Parameters",
    ...
    ax.set_title("U4 reference + all U4-lineage architecture probes (stages 4-20, 26): Dice vs. MACs",
                 color=INK, fontsize=12)
    fig.tight_layout()
    ...
    if "mem_elements" in focused.columns:
        ...
        if not mem_focused.empty:
            ...
            ax.set_title("U4 reference + all U4-lineage architecture probes (stages 4-20, 26): Dice vs. FINN buffer memory",
                         color=INK, fontsize=12)
            fig.tight_layout()
