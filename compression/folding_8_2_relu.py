"""Real folding ILP feasibility check for nnUNetTrainerENet_8_2_relu (native
width, channels 4,16,32,16,4, compression/results.csv's own
8_reginterleaved_isolation row, dice=0.8218). See
compression/folding_8_2_relu_w24_no_reg_d2_projected.py for the full
rationale (identical mechanism, this file's own CONFIG differs only in
channels/bottlenecks_per_stage/context_pattern to match this architecture's
own native-width, reg-interleaved recipe) -- not repeated here.

Usage:
    python compression/folding_8_2_relu.py
"""
...
CONFIG = dict(
    ...
    use_dsc=False, context_pattern="dense_dilation_reg_interleaved", use_prelu=False,
...
def main() -> None:
    ...
    out_file = REPO_ROOT / "compression" / "hawq" / "folding_8_2_relu_int8_100pct.json"
