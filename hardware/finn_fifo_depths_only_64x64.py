...
trained_rtl_mvau_8way_full_v3_nouram.py, for the 64x64 case -- for
apples-to-apples comparison against finn_fifo_depths_only_512x512.py.
...
Run inside the FINN container, reusing the already-completed 64x64 rtl_mvau
preamble (no need to re-run it):
    docker exec -e HOME=/tmp/home_dir <container> bash -c \\
        'cd /home/thelegendiv/finn/notebooks/enet && \\
         OUTDIR=finn_deployment_outputs/12_dense_relu_warmstart150ep_alpha025_fifo_depths_only_64x64_$(date +%Y%m%d_%H%M%S) && \\
         python3 finn_fifo_depths_only_64x64.py \\
           finn_deployment_outputs/12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_preamble_20260917_234018 \\
           /home/thelegendiv/finn/notebooks/enet/$OUTDIR \\
         > /tmp/finn_fifo_depths_only_64x64.log 2>&1'
"""
...
def _build_one_partition_fifo_only(dataflow_model_filename, cfg, prefix, folding_config_file):
    ...
    # env var back here would nest build dirs one level deeper per partition
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: finn_fifo_depths_only_64x64.py <rtl_mvau_64x64_preamble_output_dir> [explicit_output_dir]")
        sys.exit(1)
    ...
    if len(sys.argv) >= 3:
        ...
        OUTPUT_DIR = os.path.join(
            ...
            f"12_dense_relu_warmstart150ep_alpha025_fifo_depths_only_64x64_{timestamp}",
        )
    ...
    out_csv = os.path.join(OUTPUT_DIR, "fifo_depths_64x64.csv")
