...
of the FRESH/DUMMY-weight (torch.manual_seed(0)), per-block-HAWQ-bit-width
(min4 scheme), separable_dilated + dense_dilation, plain-ReLU
12_separable_dense_relu export, derive its bridged (PE, SIMD, resType=dsp,
forced on both MVAU AND VVAU nodes -- this architecture happens to have 0
VVAU/depthwise nodes, see compression/hawq/config_12_separable_dense_relu.py's
SEPARABLE_DILATED=True + USE_DSC=False, but step_force_dsp is left generic)
folding config directly from
compression/hawq/folding_nodewise_12_separable_dense_relu_min4_hardcap100_maxspeed.json
(node-level ILP output, --hard-lut --hard-bram at 100% of nominal budget,
--lut-weight 0 --bram-weight 0 "maxspeed"), then run the full per-partition
build (specialize -> fold -> apply HAWQ folding -> force DSP -> HLS/RTL
codegen -> ipgen -> FIFO depths -> stitched IP) for all 8 partitions,
combine them, and generate estimate/rtlsim reports -- all as a SINGLE
build_dataflow_cfg() call/process.

Byte-for-byte copy of finn_ooc_8_2_relu_no_reg_w16_acc2x_8way_full.py's own
logic (node-index-range partition derivation, folding-bridge incl. its
node_type/depthwise_vvau_slot dispatch, DSP-forcing, per-partition build,
combine + estimate/rtlsim reports) with MODEL_NAME/FOLDING_BLOCK_FILE/
output-dir naming changed, AND WITH ONE DELIBERATE DIFFERENCE: this file's
own steps list stops after step_measure_rtlsim_performance_multi and does
NOT attempt step_out_of_context_synthesis_multi at all. Per hardware/
finn_gotchas memory, that combined-design OOC synthesis step has hit THREE
distinct Vivado synthesis merge bugs across sessions (100% reproducible so
far) -- rather than attempt it and wait for the now-expected failure, this
build goes straight to the proven per-partition OOC synthesis workaround
(finn_ooc_12_separable_dense_relu_min4_8way_per_partition_synth.py, run
AFTER this script, using this run's own OUTPUT_DIR) as the ONLY OOC
synthesis path -- which is also, incidentally, real Vivado run one
partition at a time (sequential), never in parallel.
...
Run inside the FINN container (after the preamble has completed):
    docker exec -e HOME=/tmp/home_dir <container> bash -c \\
        ...
        finn_ooc_12_separable_dense_relu_min4_8way_full.py <preamble_output_dir> \\
        > /tmp/hawq_12_sep_dense_relu_min4_dummy_8way_full.log 2>&1 &"
"""
...
MODEL_NAME = "quantEnet_12_separable_dense_relu_min4_hawq_dummy_int8"
CONV_ORDER_FILE = os.path.join(base.ENET_DIR, f"{MODEL_NAME}_conv_order.json")
FOLDING_BLOCK_FILE = os.path.join(
    base.ENET_DIR, "folding_nodewise_12_separable_dense_relu_min4_hardcap100_maxspeed.json"
)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: finn_ooc_12_separable_dense_relu_min4_8way_full.py <hawq_preamble_output_dir>")
        sys.exit(1)
    ...
    OUTPUT_DIR = os.path.join(base.ENET_DIR, "finn_deployment_outputs", f"hawq_12_sep_dense_relu_min4_dummy_8way_full_{timestamp}")
    ...
    print("Next: run finn_ooc_12_separable_dense_relu_min4_8way_per_partition_synth.py "
          "(update its OUTPUT_DIR to the one printed above) for sequential per-partition OOC synthesis.")
