...
flat "finn_design_wrapper" top and runs SynthOutOfContext on that) has a long
history (Aug 20-21 and again now) of hitting different Verilog/Tcl bugs from
the custom merge process (swg_pkg SystemVerilog package elaboration order,
module-parameter binding loss across renamed instances, etc). All 8
partitions already have valid, individually-verified, fully-built stitched
IP (each went through its own successful CreateStitchedIP). Run FINN's own
unmodified SynthOutOfContext transform on EACH partition's OWN model
independently instead -- this is the standard, proven, un-modified FINN path
(see finn_gotchas memory note on multi-partition builds) and completely
avoids the combine-merge bug class.
"""
import dataclasses
import json
import os
