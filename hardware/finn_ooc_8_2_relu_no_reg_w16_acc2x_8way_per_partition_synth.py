"""Per-partition OOC synthesis for the w16 real-checkpoint build -- same
proven workaround already used for the w20 dummy build (see
finn_ooc_8_2_relu_no_reg_w20_acc2x_8way_per_partition_synth.py's own
docstring for the full combine-merge-bug rationale: `/memories/repo/
finn_gotchas.md`'s "THIRD distinct Verilog/Tcl merge bug" entry). Run FINN's
own unmodified SynthOutOfContext transform on EACH partition's OWN model
independently instead of the combined-design step_out_of_context_synthesis_
multi path.

IMPORTANT: OUTPUT_DIR below is a PLACEHOLDER -- update it to the actual
timestamped output_dir printed by finn_ooc_8_2_relu_no_reg_w16_acc2x_8way_
full.py (`OUTPUT_DIR= ...`) once that script has run (only needed if its own
final step_out_of_context_synthesis_multi stage fails -- if it succeeds,
this script is not needed at all).
"""
import dataclasses
import json
import os
...
OUTPUT_DIR = "/home/thelegendiv/finn/notebooks/enet/finn_deployment_outputs/hawq_8_2_w16_acc2x_dummy_8way_full_20260901_232446"
