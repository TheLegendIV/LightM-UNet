...
nnUNetTrainerENet_8_2_relu_no_reg_w20 (DSC-no-projection, dense_dilation,
plain ReLU everywhere -- USE_PRELU=False), using
compression/hawq/block_bits_8_2_relu_no_reg_w20_acc2x_joint.json's per-block
JOINT (weight_bits, act_bits) choice.

See compression/hawq/config_8_2_relu_no_reg_w20.py for the real architecture
this mirrors:
    CHANNELS = (4, 10, 20, 10, 4), BOTTLENECKS_PER_STAGE = (4, 8, 8, 2, 1)
    ...
    USE_DSC=False, DSC_NO_PROJECTION=True (unscoped/"all" -- regular1,
    stage2, stage3, regular4, regular5 all become DSC-no-projection blocks;
    down1/down2/up4/up5/final keep their normal channel-changing structure
    since DSC-no-projection cannot apply where channels change).

No trained checkpoint exists for this per-block bit-width config -- per this
session's explicit "just deploy a dummy version" / "random weights"
instruction, this uses FRESH (torch.manual_seed(0)) conv/BN weights
throughout, no PTQ/QAT calibration. Same established convention as every
other resource-probe export in this repo (finn_export_s19_hawq_block.py,
finn_export_s5_dscnoproj_dense.py, finn_enet_prod_export.py): FINN's
LUT/BRAM/DSP resource estimate and OOC synthesis result depend only on
architecture + bit-width + which nodes exist, not on weight values.

Reuses every generic FINN-safe helper from finn_export_s13_leaky_frozen.py
(_plain_relu_factory, _val, _requant_factory, export_model). This config
has USE_PRELU=False everywhere (encoder AND decoder), so no slope map / no
_make_act_factory dispatch is needed at all -- every activation site in the
network is _plain_relu_factory.

New block class: FINNDSCNoProjectionBottleneckHAWQ -- merges
finn_export_s5_dscnoproj_dense.py's FINNDSCNoProjectionBottleneck structure
(depthwise KxK groups=channels + pointwise 1x1, no reduce/expand) with
finn_export_s19_hawq_block.py's per-block (weight_bits, act_bits) +
requant-before-residual-Add pattern (needed here because neighbouring
blocks' bit-widths genuinely differ, e.g. stage3.3 w=4/a=2 vs its siblings'
w=2/a=4).

Usage (run inside the pytorch training container):
    python hardware/finn_export_8_2_relu_no_reg_w20_hawq.py

Output: hardware/outputs/finn_exports/quantEnet_8_2_relu_no_reg_w20_acc2x_hawq_dummy.onnx
Then, inside the FINN container:
    docker cp hardware/outputs/finn_exports/quantEnet_8_2_relu_no_reg_w20_acc2x_hawq_dummy.onnx \\
...
CHANNELS = (4, 10, 20, 10, 4)          # initial, s1, s23 (shared), s4, s5
BOTTLENECKS_PER_STAGE = (4, 8, 8, 2, 1)
DENSE_DILATIONS = [2, 4, 8, 16, 2, 4, 8, 16]
DEFAULT_BLOCK_BITS_FILE = (
    REPO_ROOT / "compression" / "hawq" / "block_bits_8_2_relu_no_reg_w20_acc2x_joint.json"
)
BLOCK_NAMES = (
...
class FINNDSCNoProjectionBottleneckHAWQ(nn.Module):
    """Per-block-bit-width mirror of finn_export_s5_dscnoproj_dense.py's
    FINNDSCNoProjectionBottleneck: depthwise KxK (groups=channels,
    dilation-aware) + pointwise 1x1, no reduce/expand projection. Adds the
    same requant-before-residual-Add pattern as finn_export_s19_hawq_block.py's
    FINNRegularBottleneckHAWQ since this network's neighbouring blocks'
    bit-widths genuinely differ (e.g. stage3.3 vs stage3.4)."""
...
class FINNQuantENet8w20HAWQ(nn.Module):
    """FINN-compatible, per-BLOCK-bit-width mirror of
    nnUNetTrainerENet_8_2_relu_no_reg_w20. No defaulting for missing keys --
    same philosophy every other *HAWQ model class in this repo uses."""
...
def main() -> None:
    ...
    print("\n=== Building fresh-weight, per-block-bit-width FINN-safe 8_2_relu_no_reg_w20 (HAWQ acc2x joint) ===")
    torch.manual_seed(0)
    residual = not args.no_residuals
    model = FINNQuantENet8w20HAWQ(
    ...
    name = f"quantEnet_8_2_relu_no_reg_w20_acc2x_hawq_dummy{suffix}"
