"""RTL-MVAU variant of finn_ooc_12_dense_relu_warmstart150ep_alpha025_trained_8way_full.py.
...
noActivation=1, standalone Thresholding_hls/_rtl nodes, a prerequisite for
RTL specialization) instead of the standard preamble's
...
candidatebits468_forcedsp_lut70.json under compression/MILP/artifacts/
...
"_NOACT_RTL_dspcap" filenames are retired, do not use them. Make sure the
container's copy of this file (under base.ENET_DIR) has been re-`docker cp`'d
from this path -- older container copies predate the 2026-09-17 refit.

Standalone Thresholding_hls/_rtl nodes (no PE of their own in any pre-
existing folding config) get their PE directly from this per-layer JSON's
own "thr_pe" field on the producing MVAU/VVAU's entry -- no derived/
producer-copied fixup needed (each entry's thr_pe was solved jointly with
the producer's pe/simd in the same ILP run).

Otherwise byte-for-byte identical to the non-rtl_mvau sibling: same 8-way
partitioning, same DSP forcing, same BIPOLAR weight-dtype fix, same
deliberate stop before step_out_of_context_synthesis_multi (OOC synthesis
done exclusively by finn_ooc_12_dense_relu_warmstart150ep_alpha025_8way_
per_partition_synth.py, chained via `&&` right after this script).
...
chaining full-build -> per-partition OOC synth automatically via `&&`:
    docker exec -e HOME=/tmp/home_dir <container> bash -c \\
        'cd /home/thelegendiv/finn/notebooks/enet && \\
         ...
         nohup bash -c "python3 finn_ooc_12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_8way_full.py \\
         ...
         > /tmp/12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_8way_full_and_synth.log 2>&1 &'
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: finn_ooc_12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_8way_full.py "
    ...
    if len(sys.argv) >= 3:
        ...
        OUTPUT_DIR = os.path.join(
            ...
            f"12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_8way_full_{timestamp}",
        )
