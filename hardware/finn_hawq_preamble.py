"""Fast (no-Vivado) preamble for the HAWQ per-block-bit-width S19 variant:
runs the model through step_qonnx_to_finn -> tidy -> fuse_leaky_relu_to_threshold
...
assign_stage_partition_ids_8way.onnx equivalent for the HAWQ model, mirroring
finn_relu_only_preamble.py exactly.

Run inside the FINN container:
    docker exec -e HOME=/tmp/home_dir <container> python3 \
        /home/thelegendiv/finn/notebooks/enet/finn_hawq_preamble.py
"""
import os
import sys
import dataclasses
...
MODEL_NAME = "quantEnet_s19_hawq_block_int8"
MODEL_FILE = os.path.join(base.ENET_DIR, f"{MODEL_NAME}.onnx")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_DIR = os.path.join(base.ENET_DIR, "finn_deployment_outputs", f"hawq_preamble_{timestamp}")
