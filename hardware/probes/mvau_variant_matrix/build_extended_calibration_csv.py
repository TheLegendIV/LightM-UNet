"""Build an EXTENDED per-node hardware-calibration CSV for the
mvau_variant_matrix probe run, covering not just MVAU_hls/MVAU_rtl/
VVAU_hls/VVAU_rtl but ALSO the standalone Thresholding_hls/_rtl nodes and
the ConvolutionInputGenerator_hls/_rtl (SWU) node that some combos'
topologies include upstream of a conv-like MVAU/VVAU -- so the dataset
captures the FULL per-node hardware footprint (MVAU compute + SWU +
Thresholding), not just the MVAU/VVAU multiply-accumulate core, and keeps
op_type as the column that distinguishes _hls vs _rtl implementations.

Inputs (already staged locally under hardware/temp/mvau_variant_matrix_ext/reports/):
  <label>.rpt            Vivado `report_utilization -hierarchical -hierarchical_depth 4`
  <label>_attrs_ext.json  landed nodeattrs for MVAU/VVAU/Thresholding/SWU nodes
                          (via dump_node_attrs_ext.py)

Output:
  hardware/datasets/mvau_variant_matrix_dataset_extended.csv
"""
import csv
import json
import math
...
REPORTS_DIR = HERE.parent.parent / "temp" / "mvau_variant_matrix_ext" / "reports"
OUT_DIR = HERE.parent.parent / "datasets"
...
def parse_hier_report(rpt_path: Path) -> dict:
    """Same depth-2 instance extraction as hardware/archive/
    build_alpha025_calibration_csv.py / run_variant_matrix.py."""
...
for combo in COMBOS:
    label = combo["label"]
    rpt_path = REPORTS_DIR / f"{label}.rpt"
    attrs_path = REPORTS_DIR / f"{label}_attrs_ext.json"
    if not rpt_path.exists() or not attrs_path.exists():
        print(f"SKIP {label}: missing {rpt_path.name if not rpt_path.exists() else attrs_path.name}")
        continue
    ...
    for node in attrs:
        ...
        res = hier.get(name)
        if res is None:
            print(f"  {label}: WARNING no hier row for {name!r}")
            continue
        ...
        row.update({
            ...
            "node_name": name, "op_type": op_type,
...
out_path = OUT_DIR / "mvau_variant_matrix_dataset_extended.csv"
...
print(f"wrote {len(rows)} rows ({len(COMBOS)} combos) to {out_path}")
