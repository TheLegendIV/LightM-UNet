"""Build the EXTENDED per-node hardware-calibration CSV for the
`probe_s12_context_dense_int6_pemh_simd1` probe (stem + 2x
QuantRegularBottleneck, dilations 2/4, INT6 weights/activations, folding
FORCED to PE=MH/SIMD=1), covering MVAU/VVAU AND the standalone
Thresholding_hls/_rtl + ConvolutionInputGenerator_hls/_rtl (SWU) nodes --
see repo memory `finn-container-env.md` STANDING CONVENTION note. Joins
the already-generated hier report (`hardware/temp/
hier_probe_dense_int6_pemh.rpt`) against `dump_node_attrs_ext.py` output
(`hardware/temp/attrs_ext_s12_context_dense_int6_pemh.json`) -- no
Vivado re-run needed.

Output: hardware/probes/mvau_lut_calibration_dataset_s12_context_dense_int6_pemh_simd1_extended.csv
"""
import csv
import json
import math
...
STAGE_DIR = HERE.parent / "temp"
RPT_PATH = STAGE_DIR / "hier_probe_dense_int6_pemh.rpt"
ATTRS_PATH = STAGE_DIR / "attrs_ext_s12_context_dense_int6_pemh.json"
OUT_PATH = HERE / "mvau_lut_calibration_dataset_s12_context_dense_int6_pemh_simd1_extended.csv"
...
for node in attrs:
    ...
    res = hier.get(name)
    if res is None:
        print(f"WARNING no hier row for {name!r}")
        continue

    row = {col: None for col in HEADER}
    row.update({
        "partition": 0, "node_name": name, "op_type": op_type,
