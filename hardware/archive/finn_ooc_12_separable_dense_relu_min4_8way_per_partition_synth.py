"""Per-partition OOC synthesis for the 12_separable_dense_relu_min4 8-way
builds (both the dummy-weight/hardcap100 and trained-weight/hardcap131
variants) -- same proven workaround already used for the w16/w20/26_9_w24
builds (see `/memories/repo/finn_gotchas.md`'s "THIRD distinct Verilog/Tcl
merge bug" entry). Run FINN's own unmodified SynthOutOfContext transform on
EACH partition's OWN model independently instead of the combined-design
step_out_of_context_synthesis_multi path.

OUTPUT_DIR is now taken from sys.argv[1] (the SAME timestamped output_dir
the corresponding *_8way_full.py script used) so it can be `&&`-chained
right after that script in one shell command -- reliably starting OOC
synth the moment the build finishes, unattended. Falls back to the
DEFAULT_OUTPUT_DIR constant below (the last dummy-weight build's own dir)
only if invoked with no arguments at all.
"""
import dataclasses
import json
import os
...
DEFAULT_OUTPUT_DIR = "/home/thelegendiv/finn/notebooks/enet/finn_deployment_outputs/hawq_12_sep_dense_relu_min4_dummy_8way_full_20260903_001821"
