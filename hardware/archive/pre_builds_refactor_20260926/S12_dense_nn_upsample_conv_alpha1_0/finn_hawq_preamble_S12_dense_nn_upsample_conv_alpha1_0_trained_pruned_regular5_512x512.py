"""Preamble (no-Vivado) for the S12_dense_nn_upsample_conv (alpha=1.0) REAL
TRAINED checkpoint build, WITH `regular5.0` pruned out of the graph
entirely (BOTTLENECKS_PER_STAGE n5=0 instead of 1 -- per explicit
instruction, that bottleneck was judged useless), 512x512 input. IDENTICAL
in structure/steps to finn_hawq_preamble_12_dense_relu_nearest_conv_upsample_dummy_512x512.py
...
MODEL_FILE and OUTPUT_DIR differ (pruned+trained export, not dummy).
...
Run inside the FINN container:
    docker exec -e HOME=/tmp/home_dir <container> bash -c \\
        "cd /home/thelegendiv/finn/notebooks/enet && python3 finn_hawq_preamble_S12_dense_nn_upsample_conv_alpha1_0_trained_pruned_regular5_512x512.py"
"""
import os
import sys
import dataclasses
...
OUTPUT_DIR = os.path.join(
    ...
    f"S12_dense_nn_upsample_conv_alpha1_0_trained_pruned_regular5_rtl_mvau_512x512_preamble_{timestamp}",
)
