"""v4-MILP (alpha=0.5, lut50_bram20_dspnocap), 512x512 variant of
finn_ooc_12_dense_relu_warmstart150ep_trained_rtl_mvau_8way_full_v3.py --
NOT based on the v3_nouram copy (that one has a self-mutating
FINN_BUILD_DIR bug in its own per-partition builder, never fixed).

Resumes from the 512x512 rtl_mvau preamble's
`step_enet_convert_to_hw_rtl_mvau.onnx` exactly like the v3 script (preamble
is folding-source and resolution-agnostic, reused as-is).

Folding source is now MILP alpha=0.5, lut50_bram20_dspnocap:
layer_bits_folding_12_dense_relu_warmstart150ep_joint_alpha0.5_candidatebits468_forcedsp_lut50_bram20_dspnocap.json.
Make sure this JSON has been `docker cp`'d into the container's
base.ENET_DIR before running.

Two deltas vs v3.py's per-partition builder:
  1. FIFO ram_style is NEVER forced to "ultra" (URAM) -- v3's
     step_force_fifo_uram is dropped entirely, left at FINN's own "auto"
     default for every FIFO (per explicit instruction to skip URAM
     allocation for FIFOs on this build).
  2. SplitLargeFIFOs -> PrepareIP -> HLSSynthIP is inserted right after
     step_set_fifo_depths, before CreateStitchedIP -- required for 512x512
     because auto-sized FIFO depths here can exceed Vivado FIFO Generator's
     32768-depth ceiling; SplitLargeFIFOs' newly created StreamingFIFO_rtl
     nodes have no ip_path, which CreateStitchedIP otherwise asserts on.
     Validated end to end via a real Vivado OOC synth run on partition 0,
     see hardware/temp/_resume_p0_ooc_synth.py.

Also, per explicit instruction, OOC synthesis for each partition now runs
INLINE inside that partition's own ProcessPoolExecutor worker, immediately
after CreateStitchedIP -- not deferred to the separate
finn_ooc_..._8way_per_partition_synth.py script run afterwards. Since up to
4 partitions already build concurrently, their OOC synths run concurrently
too (each partition starts synthesizing as soon as ITS OWN build finishes,
without waiting for the other 7).

Otherwise identical to v3.py: same 8-way partitioning, same DSP forcing,
same BIPOLAR weight-dtype fix, same standalone-threshold-aware accumulator
widening.

Run inside the FINN container (after the 512x512 rtl_mvau preamble has
completed):
    docker exec -e HOME=/tmp/home_dir <container> bash -c \\
        'cd /home/thelegendiv/finn/notebooks/enet && \\
         ...
         > /tmp/12_dense_relu_warmstart150ep_alpha05_trained_rtl_mvau_8way_full_v4_512x512.log 2>&1 & disown'
"""
...
# v4 MILP solve: alpha=0.5, hard LUT/BRAM fractions 50%/20%, DSP uncapped
# (forcedsp, dspnocap).
...
# block, feeding a VVAU. CORRECTION (2026-09-24): the "this architecture has
# 0 depthwise layers" assumption this comment used to make is WRONG -- the
# up4/up5 frozen bilinear-upsample blur convs (LayerQuantEnetFINN.py's
# FINNUpsamplingBottleneck) ARE real depthwise convs and DO become VVAU_hls
# nodes. That false assumption is exactly why the old node_type==
# "depthwise_vvau_slot" bridge (gated on a schema finn_milp.py's per-layer
# solver never emits) silently never fired for them: their preceding SWU's
# SIMD/parallel_window was left at FINN's own step_target_fps_parallelization
# auto-heuristic default, 4-8x faster than the MILP's chosen VVAU PE=1 --
# see repo memory finn_gotchas.md for the resulting FIFO/BRAM blowup this
# caused on partitions 5/6 of the v4_512x512 build. Fixed below: every
# VVAU_hls/VVAU_rtl node now scales its own PE UP to match its SWU's
# already-assigned SIMD (never shrinks the SWU down to the MILP's PE), and
# forces parallel_window=1 on that SWU.
...
_BASE_BUILD_DIR = "/home/thelegendiv/finn/notebooks/enet/finn_build_tmp"
...
def derive_fallback_pe_simd(logical_name, per_layer, node=None):
    ...
    nodes at all -- see finn_export_..._alpha025_trained.py's module
    docstring). Kept for parity with the separable-family bridge; in
    practice this should never fire for this architecture since every real
    MVAU/VVAU-producing module here has a per_layer entry."""
...
def find_preceding_swu_fmpad(kernel_model, vvau_node):
    """Uses real tensor connectivity (not node-list position) -- some
    depthwise VVAUs (e.g. up4/up5's 3x3 blur convs) need no explicit
    FMPadding at all (padding=0), so a fixed "2 positions before" offset is
    unreliable (confirmed: it breaks on a VVAU_hls fed by two directly-
    adjacent SWU nodes with no FMPadding between them, 2026-09-24)."""
...
def build_partition_folding_config(preamble_dir, partition_idx, sdp_node_name, partition_model_fn, logical_names, pool_names, per_layer, output_dir):
    ...
    for node, logical_name in zip(weight_nodes, logical_names):
        ...
        # Every VVAU_hls/VVAU_rtl node here is a real depthwise conv (see
        # SWU_OP_TYPES' own comment above) -- its preceding SWU already has
        # a real SIMD assigned by step_target_fps_parallelization, entirely
        # independent of this JSON's own (MILP-chosen) pe. If the SWU is
        # faster (the common case: the MILP never sees/couples to it), scale
        # this VVAU's PE UP to match rather than leaving the SWU to overrun
        # a slow VVAU -- swu_simd already divides IFMChannels==mh by
        # construction, so this can only raise pe, never produce an invalid
        # (non-divisor) PE.
        ...
        # v4's per-layer entries don't carry a plain "ram_style" key at all
        ...
        if thr_pe is not None:
            ...
            if thresh_node is not None:
                ...
                # v4's per-layer "thr_ram_style" is "block" or "distributed"
                # (never "ultra"). "distributed" forces depth_trigger_bram to
                # a large sentinel, which -- per thresholding.sv's own
                ...
                # default, i.e. FINN's own "auto" heuristic.
        ...
        if swu_node is not None:
            # PE now >= the SWU's own SIMD (scaled up above, or already
            # equal) -- both individually already valid, so the only real
            # requirement left is parallel_window=1 whenever this VVAU
            # consumes more than 1 kernel-tap per cycle (SIMD>1), so the SWU
            # actually emits the whole K*K window per cycle instead of one
            # tap at a time. SWU/FMPadding's own SIMD is never touched.
...
def step_force_dsp(model, cfg=None):
    ...
    KxK) node -- this architecture has 0 VVAU nodes (all-dense, non-
    separable dilated context), so in practice this only ever touches MVAU."""
    n_dsp = 0
...
def _build_and_synth_one_partition(dataflow_model_filename, cfg, prefix, folding_config_file, partition_idx, report_dir):
    # Own subfolder per partition (e.g. finn_build_tmp/GenericPartition_3/) --
    ...
    # matching FINN's own ZynqBuild.apply() reference ordering.
    kernel_model = kernel_model.transform(GiveUniqueNodeNames())
    kernel_model = kernel_model.transform(GiveReadableTensorNames())
    ...
    # No FIFO ram_style forcing here (URAM allocation for FIFOs skipped per
    # explicit instruction) -- every FIFO stays at FINN's own "auto" default.
    ...
    # generation for them (validated end to end via real Vivado OOC synth,
    # see hardware/temp/_resume_p0_ooc_synth.py).
    ...
    # immediately after its own build finishes -- other partitions' workers
    # do the same concurrently, so no partition waits on the other 7.
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: finn_ooc_12_dense_relu_warmstart150ep_alpha05_trained_rtl_mvau_8way_full_v4_512x512.py "
    ...
    if len(sys.argv) >= 3:
        ...
        OUTPUT_DIR = os.path.join(
            ...
            f"12_dense_relu_warmstart150ep_alpha05_trained_rtl_mvau_8way_full_v4_512x512_{timestamp}",
        )
