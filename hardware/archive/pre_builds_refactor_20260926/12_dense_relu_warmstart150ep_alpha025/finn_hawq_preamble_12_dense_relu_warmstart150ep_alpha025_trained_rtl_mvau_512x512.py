...
finn_hawq_preamble_12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau.py
except it points at the 512x512-input export
(quantEnet_12_dense_relu_warmstart150ep_alpha025_trained_int8_512x512.onnx,
produced by finn_export_12_dense_relu_warmstart150ep_alpha025_trained.py
--input-hw 512 512 -- same lut70 per-site bit-widths/checkpoint as the 64x64
build, only the input tensor's spatial size differs) and writes to its own
OUTPUT_DIR suffix so it never collides with any 64x64 preamble run.

Run inside the FINN container:
    docker exec -e HOME=/tmp/home_dir <container> python3 \\
        /home/thelegendiv/finn/notebooks/enet/finn_hawq_preamble_12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_512x512.py
"""
import os
import sys
import dataclasses
...
def _find_stage_boundaries_relaxed(model):
    ...
    nodes survive since fewer activations get fused away before
    InferConvInpGen runs -- harmless to the actual boundary computation,
    which only uses StreamingMaxPool/UpsampleNearestNeighbour node counts.

    Handles both cases seen across checkpoints: if main_up's nearest-
    neighbor Resize survives to become 2 UpsampleNearestNeighbour_* nodes
    after step_enet_convert_to_hw_rtl_mvau, use those directly; otherwise
    fall back to main_up's own FMPadding_Pixel pair (kernel padding for the
    tent conv, immediately preceded by main_act's Thresholding), which is
    still the first HW-visible op of each upsampling bottleneck either way."""
...
def _check_dangling_nodes(model):
    """Traversal-based dangling-node checker -- a node is only truly "live"
    if it's BOTH forward-reachable from a graph input AND backward-reachable
    from a graph output (on some input->output path)."""
...
MODEL_NAME = "quantEnet_12_dense_relu_warmstart150ep_alpha025_trained_int8_512x512"
MODEL_FILE = os.path.join(base.ENET_DIR, f"{MODEL_NAME}.onnx")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
OUTPUT_DIR = os.path.join(
    base.ENET_DIR, "finn_deployment_outputs",
    f"12_dense_relu_warmstart150ep_alpha025_trained_rtl_mvau_512x512_preamble_{timestamp}",
)
