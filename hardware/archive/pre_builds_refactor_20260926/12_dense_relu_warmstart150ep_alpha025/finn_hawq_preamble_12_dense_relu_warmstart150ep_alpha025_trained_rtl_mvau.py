...
def _find_stage_boundaries_relaxed(model):
    ...
    UPDATE 2026-09-16 (found on a from-scratch rerun): this checkpoint's
    main_up (both up4 and up5) is the post-2026-09-14 "tent-kernel
    depthwise conv" substitute (see LayerQuantEnetFINN.py's
    _nearest_depthwise_bilinear_kernel) -- it never produces a separate
    Resize/UpsampleNearestNeighbour_* node at all (confirmed: 0 such nodes
    at EVERY checkpoint, pre- and post-convert_to_hw, in both the rtl_mvau
    AND the standard/HLS conversion of this exact model). So the
    UpsampleNearestNeighbour-based up4_start/up5_start detection always
    finds 0, not 2 -- a real, pre-existing bug (not rtl_mvau-specific),
    just never hit before because no preamble had been rerun since the
    architecture change. Fallback: main_up's own FMPadding_Pixel pair
    (kernel padding for the tent conv, immediately preceded by main_act's
    Thresholding) is still present and still IS the first HW-visible op of
    each upsampling bottleneck (main_proj+main_act get fused away earlier,
    same as with the old Resize-based main_up) -- confirmed by inspecting
    a real converted checkpoint: FMPadding_Pixel nodes cluster into 2
    adjacent pairs (up4's, up5's) plus 1 isolated trailing one (final's
    own ConvTranspose2d), regardless of the 3-vs-5 total count. Use the
    first node of each of the first two pairs (by node-index proximity,
    gap<=5) as up4_start/up5_start; ignore any non-paired (isolated)
    FMPadding_Pixel node."""
...
def _check_dangling_nodes(model):
    """Traversal-based dangling-node checker (added 2026-09-17, after
    finding 2 genuinely-dead MVAU/Thresholding chains caused by
    step_fuse_forked_dequant_into_duplicate_threshold leaving an orphaned
    original MultiThreshold behind -- see finn_enet_build_fixups.py). A
    node is only truly "live" if it's BOTH forward-reachable from a graph
    input AND backward-reachable from a graph output (on some
    input->output path) -- forward-reachability alone is NOT sufficient:
    every node in the 2026-09-17 bug was still forward-reachable, just
    never led anywhere."""
