...
of the REAL, trained checkpoint (ft15ep) weight transfer, PER-LAYER-HAWQ-
bit-width (alpha=0.25), separable (k,1)+(1,k)-factored-dilated-context
(SEPARABLE_DILATED=True), plain-ReLU 12_separable_dense_relu export, derive
its bridged (PE, SIMD, resType=dsp, forced on both MVAU AND VVAU nodes --
this architecture, like its dense sibling, has 0 VVAU/depthwise nodes: the
(k,1)/(1,k) dilated-conv factoring is still a regular (non-depthwise) conv
per half, so step_force_dsp only ever touches MVAU here too, kept generic
for parity) folding config directly from compression/hawq/artifacts/
S12_ILP_outputs_perlayer_forcedsp_lut70/layer_bits_folding_
12_separable_dense_relu_joint_alpha0.25_candidatebits468_forcedsp_lut70.json
(per-layer ILP output, same "per_layer" logical-dotted-name schema as the
dense sibling -- confirmed this file already has separate
"stage2.N.conv.0"/"stage2.N.conv.3" keys for every dilated block, matching
this architecture's real (k,1)+(1,k)-split site names exactly, so
...
finn_ooc_12_dense_relu_warmstart150ep_alpha025_trained_8way_full.py (the
DENSE, non-separable sibling this mirrors) with only MODEL_NAME/
CONV_ORDER_FILE/FOLDING_BLOCK_FILE/output-dir naming changed. All bridge
logic (resolve_folding_entry, derive_fallback_pe_simd,
build_partition_folding_config, load_all_partition_logical_names's
weight-tensor-dedup handling for step_dedup_forked_matmul_before_threshold's
deliberate MatMul duplication, step_force_dsp, step_fix_weight_dtype_
bipolar_bug) is architecture-generic and reused verbatim.

CONV_ORDER_FILE points at
quantEnet_12_separable_dense_relu_alpha025_dummy_int8_conv_order.json
(produced by finn_hawq_dump_conv_order_12_separable_dense_relu_alpha025.py)
...
VALUES differ); no need to regenerate it for the trained export.

Same DELIBERATE difference as the dense sibling: this file's own steps list
stops after step_measure_rtlsim_performance_multi and does NOT attempt
step_out_of_context_synthesis_multi at all (that combined-design path has
hit 3 distinct Vivado merge bugs across sessions -- see
/memories/repo/finn_gotchas.md). OOC synthesis is done exclusively by
finn_ooc_12_separable_dense_relu_alpha025_8way_per_partition_synth.py,
...
(finn_hawq_preamble_12_separable_dense_relu_alpha025_trained.py)
...
OOC synth reliably starts the moment the build finishes, unattended:
    docker exec -e HOME=/tmp/home_dir <container> bash -c \\
        'cd /home/thelegendiv/finn/notebooks/enet && \\
         OUTDIR=finn_deployment_outputs/12_separable_dense_relu_alpha025_trained_8way_full_$(date +%Y%m%d_%H%M%S) && \\
         nohup bash -c "python3 finn_ooc_12_separable_dense_relu_alpha025_trained_8way_full.py \\
           <preamble_output_dir> /home/thelegendiv/finn/notebooks/enet/\$OUTDIR && \\
           python3 finn_ooc_12_separable_dense_relu_alpha025_8way_per_partition_synth.py \\
           /home/thelegendiv/finn/notebooks/enet/\$OUTDIR" \\
         > /tmp/12_separable_dense_relu_alpha025_trained_8way_full_and_synth.log 2>&1 &'
"""
...
CONV_ORDER_FILE = os.path.join(base.ENET_DIR, "quantEnet_12_separable_dense_relu_alpha025_dummy_int8_conv_order.json")
FOLDING_BLOCK_FILE = os.path.join(
    base.ENET_DIR, "layer_bits_folding_12_separable_dense_relu_joint_alpha0.25_candidatebits468_forcedsp_lut70.json"
)
...
# kept for parity with the dense-sibling bridge logic).
...
def load_all_partition_logical_names(preamble_dir):
    ...
    # same tensor (correct -- it's literally the same conv/weight, just
    # legitimately re-emitted for a second consumer).
...
def derive_fallback_pe_simd(logical_name, per_layer, node=None):
    ...
    docstring). Kept for parity with the dense-sibling bridge; in practice
    this should never fire for this architecture since every real
...
def step_force_dsp(model, cfg=None):
    ...
    KxK) node -- this architecture has 0 VVAU nodes (the (k,1)+(1,k)
    dilated-conv factoring is still a regular, non-depthwise conv per half),
    so in practice this only ever touches MVAU."""
    n_dsp = 0
...
def main():
    if len(sys.argv) < 2:
        print("Usage: finn_ooc_12_separable_dense_relu_alpha025_trained_8way_full.py "
    ...
    if len(sys.argv) >= 3:
        ...
        OUTPUT_DIR = os.path.join(
            ...
            f"12_separable_dense_relu_alpha025_trained_8way_full_{timestamp}",
        )
    ...
    print("Next: finn_ooc_12_separable_dense_relu_alpha025_8way_per_partition_synth.py will run "
