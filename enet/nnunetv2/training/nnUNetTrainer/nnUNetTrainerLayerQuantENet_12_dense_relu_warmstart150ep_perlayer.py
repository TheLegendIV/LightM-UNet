"""QAT trainer for nnUNetTrainerENet_12_dense_relu_warmstart150ep's per-LAYER
...
per-site naming scheme). Byte-for-byte the same mechanism as
nnUNetTrainerLayerQuantENet_12_separable_dense_relu_perlayer, just for the
WARM-STARTED 12_dense_relu architecture (separable_dilated=False -- dense
KxK dilated convs, plain "dense_dilation" context pattern, no reg-trailing
block -- see compression/hawq/config_12_dense_relu_warmstart150ep.py). Same
construction/Brevitas-device-fix pattern as every other CombinedQuantENet_*/
LayerQuantENet_* trainer.
"""
...
class nnUNetTrainerLayerQuantENet_12_dense_relu_warmstart150ep_perlayer(nnUNetTrainerENet):
    ...
    def build_network_architecture(
        ...
        if len(configuration_manager.patch_size) != 2:
            raise ValueError(
                "nnUNetTrainerLayerQuantENet_12_dense_relu_warmstart150ep_perlayer is a 2D architecture. "
        ...
        common_kwargs = dict(
            ...
            use_dsc=False, dsc_no_projection=False, separable_dilated=False, trainable_slope=False,
        )
