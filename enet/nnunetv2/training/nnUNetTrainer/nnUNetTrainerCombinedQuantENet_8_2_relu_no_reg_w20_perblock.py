"""QAT trainer for nnUNetTrainerENet_8_2_relu_no_reg_w20's own real
per-BLOCK HAWQ bit assignment (compression/hawq/block_bits_8_2_relu_no_reg_
w20_acc1x_joint.json -- one independent (weight_bits, act_bits) pair per
individual bottleneck block, 29 entries, sensitivity_weight=1.0 "acc1x" --
found Optimal: all 16 context blocks at uniform 2-bit weights, mixed
2/4-bit activations, real folding shows 100.0% LUT / 25.8% BRAM / ~23.0ms
hard-capped -- the fastest deployable result of any S8.2 variant checked
this session, at channels=(4,10,20,10,4), the narrowest width tested).

Same construction/Brevitas-device-fix pattern as
nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_fullwidth_perblock (see that
file's own module docstring for the full rationale -- not repeated here)
-- this trainer differs only in CHANNELS (4,10,20,10,4) and always
warm-starts from compression/post-quantization/calibrate_8_2_relu_no_reg_
w20_perblock.py's own calibrated checkpoint via ENET_PRETRAINED_CHECKPOINT.
"""
...
class nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_w20_perblock(nnUNetTrainerENet):
    ...
    def build_network_architecture(
        ...
        if len(configuration_manager.patch_size) != 2:
            raise ValueError(
                "nnUNetTrainerCombinedQuantENet_8_2_relu_no_reg_w20_perblock is a 2D architecture. "
