"""QAT trainer for nnUNetTrainerENet_12_separable_dense_relu_warmstart150ep's
...
the WARM-STARTED 12_separable_dense_relu architecture (separable_dilated=
True -- dense conv factored into a (k,1)+(1,k) pair, plain "dense_dilation"
context pattern, no reg-trailing block -- see compression/hawq/
config_12_separable_dense_relu_warmstart150ep.py). Same construction/
Brevitas-device-fix pattern as every other CombinedQuantENet_*/
...
class nnUNetTrainerLayerQuantENet_12_separable_dense_relu_warmstart150ep_perlayer(nnUNetTrainerENet):
    ...
    def build_network_architecture(
        ...
        if len(configuration_manager.patch_size) != 2:
            raise ValueError(
                "nnUNetTrainerLayerQuantENet_12_separable_dense_relu_warmstart150ep_perlayer is a 2D "
